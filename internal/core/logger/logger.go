...
func NewLogger(config Config) (*Logger, error) {
	...
	if err := zapLevel.UnmarshalText([]byte(config.Level)); err != nil {
		return nil, fmt.Errorf("unmashal log levl: %w", err)
	}
	...
	timestemp := time.Now().UTC().Format("2006-01-02T15-04-05.000000")
	logFilePath := filepath.Join(config.Folder, fmt.Sprintf("%s, log", timestemp))
